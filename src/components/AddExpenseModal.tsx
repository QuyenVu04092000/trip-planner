    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white w-full max-w-sm rounded-t-3xl sm:rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 pt-5 pb-3 border-b border-sand">
          <h2 className="font-semibold text-ink">{isEdit ? 'Chỉnh sửa chi tiêu' : 'Thêm chi tiêu'}</h2>
        ...
        <div className="p-4 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          {hasFunds && (
            <div className="flex gap-2 p-1 bg-parchment rounded-xl">
              <button
          ...
          {paySource === 'fund' && funds.length > 1 && (
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-stone uppercase tracking-wide">Chọn quỹ</label>
              <select
                value={selectedFundId}
                onChange={e => setSelectedFundId(e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl border border-gold/40 text-sm focus:outline-none focus:ring-2 focus:ring-gold/40 bg-white"
              >
                {funds.map(f => (
          ...
          {paySource === 'fund' && selectedFund && fundCollector && (
            <div className="flex items-center gap-2 px-3 py-2 bg-gold-pale rounded-xl border border-gold/30">
              <PiggyBank size={14} className="text-gold flex-shrink-0" />
              <div className="min-w-0">
                <p className="text-xs font-semibold text-gold-dark truncate">{selectedFund.description}</p>
          ...
          {/* Description */}
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-stone uppercase tracking-wide">Mô tả</label>
            <input
              type="text"
              value={description}
          ...
          {/* Amount */}
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-stone uppercase tracking-wide">Số tiền (đ)</label>
            <input
              type="text"
              inputMode="numeric"
              value={amount}
          ...
          {paySource === 'personal' && (
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-stone uppercase tracking-wide">Người trả</label>
              <select
                value={paidBy}
                onChange={e => setPaidBy(e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl border border-sand text-sm focus:outline-none focus:ring-2 focus:ring-terra/40 bg-white"
              >
                {members.map(m => (
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-stone uppercase tracking-wide">Cách chia</label>
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-sm font-medium border transition-colors ${
                  splitType === 'equal'
                    ? 'bg-terra text-white border-terra-dark'
                    : 'bg-white text-stone border-sand'
                }`}
              >
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-colors ${
                        checked ? 'bg-terra-pale border border-terra/25' : 'bg-paper border border-transparent'
                      }`}
                    >
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                        checked ? 'border-terra bg-terra' : 'border-sand-dark'
                      }`}>
          ...
          {splitType === 'custom' && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-medium text-stone uppercase tracking-wide">Số tiền từng người</label>
                {numAmount > 0 && (
                  <span className={`text-xs font-semibold ${customRemainder === 0 ? 'text-sage-dark' : 'text-wine'}`}>
                    {customRemainder === 0
                      ? '✓ Khớp'
                      : customRemainder > 0
                ...
                {members.map(m => (
                  <div key={m.userId} className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      placeholder="0"
                      className="w-28 px-2.5 py-1.5 rounded-xl border border-sand text-sm text-right focus:outline-none focus:ring-2 focus:ring-terra/40"
                    />
                  </div>
                ))}
              </div>
