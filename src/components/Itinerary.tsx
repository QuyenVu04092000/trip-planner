...
const PALETTE = [
  { pill: 'bg-blue-500',    light: 'bg-blue-50',    accent: 'text-blue-600',    border: 'border-blue-200',    time: 'bg-blue-100 text-blue-700',    dot: 'bg-blue-500',    line: 'bg-blue-100' },
  { pill: 'bg-violet-500',  light: 'bg-violet-50',  accent: 'text-violet-600',  border: 'border-violet-200',  time: 'bg-violet-100 text-violet-700',  dot: 'bg-violet-500',  line: 'bg-violet-100' },
  { pill: 'bg-emerald-500', light: 'bg-emerald-50', accent: 'text-emerald-600', border: 'border-emerald-200', time: 'bg-emerald-100 text-emerald-700', dot: 'bg-emerald-500', line: 'bg-emerald-100' },
  { pill: 'bg-amber-500',   light: 'bg-amber-50',   accent: 'text-amber-600',   border: 'border-amber-200',   time: 'bg-amber-100 text-amber-700',   dot: 'bg-amber-500',   line: 'bg-amber-100' },
  { pill: 'bg-rose-500',    light: 'bg-rose-50',    accent: 'text-rose-600',    border: 'border-rose-200',    time: 'bg-rose-100 text-rose-700',    dot: 'bg-rose-500',    line: 'bg-rose-100' },
  { pill: 'bg-teal-500',    light: 'bg-teal-50',    accent: 'text-teal-600',    border: 'border-teal-200',    time: 'bg-teal-100 text-teal-700',    dot: 'bg-teal-500',    line: 'bg-teal-100' },
] as const;

type Palette = typeof PALETTE[number];
type FormData = Omit<Activity, 'id' | 'tripId' | 'createdAt'>;

const BLANK: FormData = { date: '', time: '', activity: '', address: '', cost: '', notes: '', position: 0 };

interface Props {
  activities: Activity[];
  startDate?: string;
  onAdd:    (fields: Omit<Activity, 'id' | 'tripId' | 'createdAt'>) => Promise<void>;
  onUpdate: (id: string, fields: Partial<Omit<Activity, 'id' | 'tripId' | 'createdAt'>>) => Promise<void>;
...
function nextDefaultDate(activities: Activity[], startDate: string): string {
  const validDates = activities.map(a => a.date).filter(isValidDate).map(normDate).sort();
...
function Card({ act, color, onEdit, onDelete }: {
  act: Activity; color: Palette; onEdit: () => void; onDelete: () => void;
}) {
  return (
    <div
      onClick={onEdit}
    ...
    >
      ...
      <div className="flex-shrink-0 w-14 pt-0.5">
        {act.time
          ? <span className={`block text-center ${color.time} text-xs font-bold px-1.5 py-1 rounded-lg`}>{act.time}</span>
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-sm text-slate-800 leading-snug">
          {act.activity || <span className="text-slate-300 font-normal italic">Chưa có tên</span>}
        </p>
        {(act.address || act.cost || act.notes) && (
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
            ...
            {act.cost && (
              <span className="text-xs font-semibold text-emerald-600">{act.cost}đ</span>
            )}
            {act.notes && (
              <span className="text-xs text-slate-400 italic truncate max-w-[140px]">{act.notes}</span>
            )}
          </div>
        )}
      </div>
...
function EditForm({ form, color, saving, onChange, onSave, onCancel }: {
  form: FormData; color: Palette; saving: boolean;
  onChange: (f: FormData) => void; onSave: () => void; onCancel: () => void;
}) {
  ...
  return (
    <div className={`${color.light} border ${color.border} rounded-2xl p-4 space-y-3.5 shadow-sm`}>
      {/* Date + Time — stack on mobile, side-by-side on sm+ with proportional widths */}
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="sm:flex-[3]">
          <span className="text-xs font-semibold text-slate-500 block mb-1.5">Ngày</span>
          <DatePicker value={form.date} onChange={v => onChange({ ...form, date: v })} placeholder="Chọn ngày" />
        </div>
        <div className="sm:flex-[2]">
          <span className="text-xs font-semibold text-slate-500 block mb-1.5">Thời gian</span>
          <TimePicker value={form.time} onChange={v => onChange({ ...form, time: v })} />
        </div>
      </div>

      {/* Activity name */}
      <div>
        <span className="text-xs font-semibold text-slate-500 block mb-1.5">Hoạt động</span>
        <input
          autoFocus
          className={inp}
          value={form.activity}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <span className="text-xs font-semibold text-slate-500 block mb-1.5">Chi phí</span>
          <input
            className={inp}
            value={form.cost}
        ...
        <div>
          <span className="text-xs font-semibold text-slate-500 block mb-1.5">Ghi chú</span>
          <input
            className={inp}
            value={form.notes}
...
export default function Itinerary({ activities, startDate = '', onAdd, onUpdate, onDelete }: Props) {
  ...
  const sortedDates = [...dateMap.keys()].sort((a, b) => normDate(a).localeCompare(normDate(b)));
  for (const acts of dateMap.values()) acts.sort((a, b) => a.time.localeCompare(b.time));

  const startEdit = (a: Activity) => {
    setAddDate(null);
    setEditId(a.id);
    setForm({ date: a.date, time: a.time, activity: a.activity, address: a.address, cost: a.cost, notes: a.notes, position: a.position });
  };
  ...
  const cancel = () => { setEditId(null); setAddDate(null); setForm(BLANK); };
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 pb-28">
      ...
      {isEmpty && (
        <div className="text-center py-20">
          ...
          <p className="text-slate-400 text-sm mb-8 max-w-xs mx-auto leading-relaxed">
            Bắt đầu lên kế hoạch từng ngày, địa điểm và chi phí cho chuyến đi của bạn.
          </p>
          <button
            onClick={() => startAdd('')}
      ...
      <div className="space-y-10">
        ...
        {sortedDates.map((date, di) => {
          ...
          const isDayEditing  = isAddingHere || acts.some(a => a.id === editId);

          return (
            <section key={date}>
              {/* Day header */}
              <div className="flex items-center gap-2.5 mb-5">
                <span className={`${color.pill} text-white text-xs font-bold px-3 py-1.5 rounded-full flex-shrink-0 shadow-sm`}>
                  Ngày {di + 1}
                </span>
                <span className={`text-sm font-semibold ${color.accent} truncate`}>{fmtDate(date) || date}</span>
                <div className="flex-1 h-px bg-slate-100 min-w-4" />
                {dayTotal > 0 && (
                  <span className="text-xs text-slate-400 font-semibold flex-shrink-0">{fmtMoney(dayTotal)}</span>
                )}
              </div>
                ...
                <div className="space-y-3">
                  {acts.map(act =>
                    editId === act.id ? (
                      ...
                      <div key={act.id} className="flex gap-3 items-start relative z-[1]">
                        {/* Timeline dot */}
                        <div className={`flex-shrink-0 w-3 h-3 rounded-full mt-4 ring-2 ring-white shadow-sm ${color.dot}`} />
                        {/* Card */}
                        <div className="flex-1 min-w-0">
                          <Card act={act} color={color} onEdit={() => startEdit(act)} onDelete={() => onDelete(act.id)} />
                        </div>
                      </div>
                    )
                  )}
                  ...
                  {isAddingHere && (
                    <EditForm form={form} color={color} saving={saving} onChange={setForm} onSave={save} onCancel={cancel} />
                  )}
                </div>
              </div>
        ...
        {ungrouped.length > 0 && (
          <section>
            <div className="flex items-center gap-2.5 mb-5">
              <span className="bg-slate-300 text-white text-xs font-bold px-3 py-1.5 rounded-full flex-shrink-0">Chưa có ngày</span>
              <div className="flex-1 h-px bg-slate-100" />
            </div>
            <div className="space-y-3">
              {ungrouped.map(act =>
                editId === act.id ? (
                  <EditForm key={act.id} form={form} color={PALETTE[sortedDates.length % PALETTE.length]} saving={saving} onChange={setForm} onSave={save} onCancel={cancel} />
                ) : (
                  <div key={act.id} className="flex gap-3 items-start">
                    <div className="flex-shrink-0 w-3 h-3 rounded-full mt-4 ring-2 ring-white bg-slate-300" />
                    <div className="flex-1 min-w-0">
                      <Card act={act} color={PALETTE[sortedDates.length % PALETTE.length]} onEdit={() => startEdit(act)} onDelete={() => onDelete(act.id)} />
                    </div>
                  </div>
                )
              )}
        ...
        {grandTotal > 0 && (
          <div className="rounded-2xl bg-slate-900 px-5 py-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] text-white/40 font-bold uppercase tracking-widest mb-1">Tổng chi phí ước tính</p>
              <p className="text-2xl font-bold text-white">{fmtMoney(grandTotal)}</p>
            </div>
