...
export function checkAndNotify(trips: Trip[]): void {
  ...
  for (const trip of trips) {
    ...
    let body = '';
    if      (daysLeft === 0) body = `${trip.emoji} ${trip.name} bắt đầu hôm nay! Chúc chuyến đi vui vẻ 🎉`;
    else if (daysLeft === 1) body = `${trip.emoji} ${trip.name} — còn đúng 1 ngày nữa là xuất phát!`;
    else if (daysLeft === 3) body = `${trip.emoji} ${trip.name} — còn 3 ngày nữa, đã chuẩn bị chưa?`;
    else if (daysLeft === 7) body = `${trip.emoji} ${trip.name} — còn 1 tuần nữa là đi rồi!`;

    if (body) {
      try {
        new Notification('TripMemo ✈️', { body, tag: notifId, icon: '/favicon.ico' });
